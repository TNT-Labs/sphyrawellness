 ...
 * 
 * VERSIONE AGGIORNATA: Include coordinamento con dbBridge per prevenire loop
 */
 ...
 * AGGIORNATO: Coordina con dbBridge per prevenire loop
 */
 ...
 * AGGIORNATO: Coordina con dbBridge per prevenire traffico inutile
 */
    ...
    Object.entries(localDatabases).forEach(([key, localDB]) => {
      ...
      try {
        ...
        sync.on('change', (info) => {
          ...
          // IMPORTANTE: Sync changed documents to IndexedDB con coordinamento dbBridge
        ...
        sync.on('paused', async () => {
          ...
          if (!initialSyncCompletedDatabases.has(key)) {
            ...
            try {
              // IMPORTANTE: Copy all documents con coordinamento dbBridge
