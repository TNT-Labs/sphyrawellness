...
Write-Host "1️⃣ Generazione chiave privata..." -ForegroundColor Cyan
...
Write-Host "2️⃣ Generazione CSR..." -ForegroundColor Cyan
...
Write-Host "3️⃣ Creazione configurazione SAN..." -ForegroundColor Cyan
$opensslConfig = @"
[req]
...
Write-Host "4️⃣ Generazione certificato self-signed (valido 10 anni)..." -ForegroundColor Cyan
openssl x509 -req `
...
Write-Host "⚠️  IMPORTANTE - Importa il certificato nel browser!" -ForegroundColor Yellow
Write-Host ""
Write-Host "Il certificato è self-signed, quindi i browser mostreranno un warning."
...
Write-Host "   2. Tab 'Autorità di certificazione radice attendibili'"
Write-Host "   3. Import → Seleziona: $CERT_DIR\sphyra.crt"
Write-Host "   4. ✓ Considera attendibile il certificato per identificare i siti web"
Write-Host ""
...
Write-Host "   3. Tab 'Autorità'"
Write-Host "   4. Importa → Seleziona: $CERT_DIR\sphyra.crt"
Write-Host "   5. ✓ Considera attendibile questa CA per identificare i siti web"
Write-Host ""
...
if ($reply -match "^[SsYy]") {
    ...
    if ($isAdmin) {
        ...
        Write-Host "⚠️  Riavvia PowerShell come Amministratore per installare il certificato" -ForegroundColor Yellow
